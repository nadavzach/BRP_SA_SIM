...
public:
    ...
    vector<vector<node_pu_os<T>>> nodes;

    grid_os (uint16_t dim, uint8_t threads, uint16_t max_depth=0);
...
template <typename T>
grid_os<T>::grid_os (uint16_t dim, uint8_t threads, uint16_t max_depth) : _dim(dim), _threads(threads), _max_depth(max_depth) {
    mem_a.reserve(dim);
    mem_b.reserve(dim);
    nodes.reserve(dim);
    ...
    for (uint8_t i=0; i<dim; i++) {
        ...
        nodes.push_back(vector<node_pu_os<T>>());
        nodes[i].reserve(dim);

        for (uint8_t j=0; j<dim; j++) {
            nodes[i].push_back(node_pu_os<T>("node_" + to_string(i) + "_" + to_string(j), threads, max_depth));
        }
    }
...
template <typename T>
void grid_os<T>::push(xt::xarray<T> &a, xt::xarray<T> &b, uint8_t thread, bool pad) {
