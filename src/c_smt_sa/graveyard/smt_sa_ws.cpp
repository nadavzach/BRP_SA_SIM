...
public:
    ...
    smt_sa_ws (uint16_t dim, uint8_t threads, uint16_t max_depth=4096);
...
template <typename T>
smt_sa_ws<T>::smt_sa_ws (uint16_t dim, uint8_t threads, uint16_t max_depth) : _dim(dim), _threads(threads), _max_depth(max_depth), sa_grid(dim, threads, max_depth), cycles(0) {}

template <typename T>
void smt_sa_ws<T>::set_inputs(xt::xarray<T> a, xt::xarray<T> b) {
...
template <typename T>
void smt_sa_ws<T>::get_tile(vector<xt::xarray<T>> &tile_a, vector<xt::xarray<T>> &tile_b, tile_idx t_idx) {
...
template <typename T>
void smt_sa_ws<T>::_subtile_dict(vector<uint16_t> &subtile_start, vector<uint16_t> &subtile_end) {
    for (uint8_t t=0; t<_threads; t++) {
        uint16_t start, end;
...
template <typename T>
void smt_sa_ws<T>::_subtile_range(uint8_t thread, uint16_t &thread_tile_start, uint16_t &thread_tile_end) {
...
template <typename T>
xt::xarray<T> smt_sa_ws<T>::go(vector<tile_idx> &tile_vec) {
