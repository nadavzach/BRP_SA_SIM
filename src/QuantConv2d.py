        ...
        if not self._unfold:
            ...
            if not self._hw_sim:
                if self._my_hw_sim: # cpy_smt_sa hw simulation
                    ...
                    b_w = a_c
                    b_h = 50
                    x_unf = np.array(x_unf)
                    w_unf = np.array(w_unf)
                    mean = np.mean(x_unf)
                    ...
                    np.savez_compressed("{}/{}.npz".format('./src/cpy_smt_sa/tests/a_b_mats', 'a_mat'+str(self._get_name())+"_"+str(self.index)), values)
                    mean = np.mean(w_unf)
                    ...
                    np.savez_compressed("{}/{}.npz".format('./src/cpy_smt_sa/tests/a_b_mats', 'b_mat'+str(self._get_name())+"_"+str(self.index)), values)
